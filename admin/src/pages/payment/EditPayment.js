...
const EditPayment = () => {
  ...
  const getPayment = async () => {
    ...
    setSiswa(response.data.siswa.nama)
    setAdmin(response.data.admin.username)
    setBulan(response.data.bulan_bayar)
    setTahun(response.data.tahun_bayar)
    setJumlah(response.data.jumlah_bayar)
  }
  ...
  const saveData = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.patch(`http://localhost:5000/payment/${id}`, {
        id_siswa: siswa,
        id_admin: admin,
        jumlah_bayar: jumlah,
        bulan_bayar: bulan,
...
export default adminLayout(EditPayment);
